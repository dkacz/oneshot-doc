...
1. **Opening box** (`KeyBox`): what the source material concerns (title and scope) and the three most important conclusions.
2. **Thematic sections** (not chronological): group by problems, not by source chapters; headings carry conclusions.
3. **Scope boundaries**: a short section or paragraph naming important source-material threads outside the agreed scope, so the reader knows the boundaries.
4. **Minimal bibliography**: full description of the source material plus any works cited by it that the summary quotes directly.
