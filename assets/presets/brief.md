...
Default page budget: 4 (the contract may change it). qmd YAML: `documentclass: scrartcl`, `pdf-engine: lualatex`, `fig-pos: "tb"`, `colorlinks: true`, and for Polish documents add `\renewcommand{\figurename}{Wykres}` in `include-before-body`.

## Skeleton

1. **Opening box** (`KeyBox`, title "W skrócie" or equivalent): two prose paragraphs in abstract style. The first says what was checked and on what basis; the second gives the result with numbers. No bullets that carry many claims at once.
2. **3-4 sections**, each with a heading that carries the section thesis, not a label ("Co pokazują dane: dług wyżej, nie niżej", not "Wyniki").
...
- Numbers in text and figures must be identical in notation (decimal comma in Polish).
